...
void indexing(TrieNode** clus_root, char* folder_name, int world_rank, int world_size ){
    ...
    *clus_root = get_clus_Node();
    ...
    int size;
    char** list_of_stopwords = read_arr(fptr, &size);
    fclose(fptr);
    ...
    for(i = subdomain_start; i < subdomain_end; i ++){
        ...
        while(1){
            ...
            if(search(list_of_stopwords, word_buffer, 0, 570) == 1){
                free(word_buffer);   
                continue;  
            }
    ...
    // Serializing the trie for the communication 
    ...
    // Gathering of all the sizes of serialized trie. (Used in gathering the trie itself)
    ...
    // Preparation before gathering every trie.
    ...
    if(world_rank == 0){
        // Decerializing the tries at root node.
        ...
        // Making a Global trie, hence a global word_document index.
        for(i = 1; i < world_size; i++){
